...
- ✅ **DOI 缓存**：跳过已处理的 DOI，无需网络请求（Go）
- ✅ **安全收割脚本**：渐进式批量下载，异常自动停止（Go）
...
- **随机延迟**：每次请求前随机延迟 1.0-4.0 秒，避免被识别为爬虫
- **自动重试**：遇到 403 错误时自动重试最多 3 次，延迟递增
- **Referer 头**：PDF 下载时添加 Referer 头，表明来源页面
- **低并发默认设置**：默认 2-3 个 workers，最小化触发频率限制的风险

### DOI 缓存（Go）

Go 版本在 `pdf/` 目录下维护两个缓存文件，**无需网络请求**即可跳过已处理的 DOI：

- `downloaded.txt` - 已成功下载的 DOI
- `not_available.txt` - 确认在 Sci-Hub 上不可用的 DOI
...
大规模下载推荐使用 `scripts/harvest.sh` 进行渐进式安全下载：

```bash
# 默认：每批 30 个，批次间随机等待 60-180 秒
./scripts/harvest.sh -a ./archive

# 自定义：每批 50 个，最多 10 轮，间隔 2-5 分钟
./scripts/harvest.sh -a ./archive -b 50 -r 10 -d 120 -D 300
```

特性：
- **异常自动停止**：连续 2 轮高错误率（≥80% 且成功 <2）自动停止
- **随机延迟**：批次间模拟人类行为
- **实时输出**：流式显示进度条和统计
- **优雅退出**：`Ctrl+C` 显示统计汇总
...
- [x] **DOI 缓存**：跳过已处理的 DOI，无需网络请求（Go）
- [x] **安全收割脚本**：渐进式批量下载，异常自动停止（Go）
